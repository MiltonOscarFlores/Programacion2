...
class Article:
    ...
    def __init__(self, nombre: str, costo: float, descuento: float = 0.0):
        self.nombre = nombre
        self.costo = costo
    ...
    def precio(self) -> float:
        precioConIva = self.costo * (1 + self.iva)
        precioConDescuento = precioConIva * (1 - self.descuento)
        return round(precioConDescuento, 2)

    @precio.setter
    def precio(self, value):
        raise AttributeError("No se puede modificar el precio")

    @classmethod
    def actualizar_iva(cls, nuevoIva: float):
        cls.iva = nuevoIva
